...
class LightGCNBase(object):
    ...
    def _convert_sp_mat_to_sp_tensor(X):
        coo = X.tocoo()
        i = torch.LongTensor([coo.row, coo.col]).to(X.dtype)
        v = torch.from_numpy(coo.data).float().to(X.dtype)
    ...
    def _base_init(self, args, corpus):
        ...
        self.sparse_norm_adj = self._convert_sp_mat_to_sp_tensor(self.norm_adj).to(self.device)  # 转换并移动到设备
        self._base_define_params()
        self.apply(self.init_weights)
    
    def _base_define_params(self):    
        self.encoder = LGCNEncoder(self.user_num, self.item_num, self.emb_size, self.sparse_norm_adj, self.n_layers).to(self.device)
...
class LGCNEncoder(nn.Module):
    def __init__(self, user_count, item_count, emb_size, sparse_norm_adj, n_layers=3):
        super(LGCNEncoder, self).__init__()
        self.user_count = user_count
        self.item_count = item_count
        self.emb_size = emb_size
        self.layers = [emb_size] * n_layers
        self.sparse_norm_adj = sparse_norm_adj  # 已经是 torch.sparse.FloatTensor 并且移动到设备
